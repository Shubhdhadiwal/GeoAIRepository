...
try:
    ...
    st.warning("Earth Engine initialization error (if this appears repeatedly, ignore if running locally): " + str(e))
...
# --- Now insert the interactive map inside the Python Codes (GEE) tab ---

if selected_tab == "Python Codes (GEE)":
    st.markdown("---")
    st.header("🗺️ Interactive Earth Engine Map")

    # Define area of interest roughly covering India
    aoi = ee.Geometry.Rectangle([68, 7, 97, 37])
    ...
    m = geemap.Map(center=[20, 78], zoom=5)
    m.add_basemap("OpenTopoMap")
    m.addLayer(image, vis_params, "Sentinel-2 RGB 2020")
